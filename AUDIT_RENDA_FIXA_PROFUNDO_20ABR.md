...
> **Status entrega (20/04/2026):** P0 (13 tickets) **✅ concluído** em commits `cdfff09` (feat RF P0 batch — 9 files, +2519/-87: NarrativeSection em 5 sections + DataAsOfStamp sticky header + KPI_HINTS RF + RfRollingGrid + CreditCalendarHeatmap NTN-B 2035 + Breakeven 3a + TesouroSimulator + RfPortfolioCalculator + CreditoPrivadoDeepPanel AA×A + ExportPdfButton + PrintFooter + no-print tags) + `30729a8` (feat RF P0-2/P0-6 follow-up — 3 files: MacroChart CSV export pt-BR upgrade + SpreadMonitor migration para csvExport helper + audit doc status banner). Build clean (tsc 0 errors, vite build 5.40s). HubRendaFixa chunk 97.55 kB / gzip 25.66 kB — **folga 38% vs target <160 kB**. Pattern parity 4/4 packs com Crédito/Fundos. Push pendente (credenciais GitHub indisponíveis nesta sessão — Lucas deve rodar `git push origin main`).
>
...
>
> **P1 Selective Batch (20/04/2026)**: Entregues 2 dos 8 tickets P1 (frontend-only, zero backend novo) em commit `27cc543` (+121/-1 em `HubRendaFixa.tsx`). **P1-1 COPOM overlay** aplicado em 4 charts críticos (Selic vs CDI, DI 30d×360d, NTN-B 2029×2035, IMA-B Proxy) via `useMonetaryEvents("COPOM")` + `MacroChartEvent` prop — pattern idêntico ao Crédito P1-2 e Macro #14. **P1-6 Cross-module navigation** com 3 superfícies de deep-link: (i) strip "Navegação cruzada" com 4 cards em Visão Geral apontando para `/hub/macro?section=monetaria|inflacao` e `/hub/credito?section=preco|volume` preservando `?period=`; (ii) pills inline em Taxas & Curva linkando para `/hub/macro?section=monetaria|expectativas`; (iii) pills inline em Crédito Privado linkando para `/hub/credito?section=preco|risco`. Todas as superfícies carregam `no-print` class. Build clean (tsc 0 errors, vite 5.16s). Chunk HubRendaFixa 97.55 → 101.14 kB / gzip 25.66 → 26.29 kB (+3.59 kB para 2 features). Deferidos pós-beta: P1-2 share-link state, P1-3 drill-down drawer, P1-4 Focus correlation heatmap + stress matrix, P1-5 DI×Pré real vertices, P1-7 alertas dinâmicos, P1-8 IMA-B real (requer licença ANBIMA). Push pendente junto com os commits P0 (`cdfff09` + `30729a8` + `bb637eb`).
>
> **P2 Polish Batch (20/04/2026)**: Entregues 6 dos 8 tickets P2 em commit `c2bb033` (3 files, +116/-36 + `src/lib/unitNormalize.ts` NEW). **P2-1 Unit consistency sweep**: `normalizeToBi(points, meta?.unit)` aplicado em Estoque TD (990201), Vendas TD (990202) e Emissões Deb. (990303) — elimina drift entre séries publicadas em "R$ milhões" vs "R$ bi" no hub_macro_series_meta. Helper extraído para `src/lib/unitNormalize.ts` (compartilhado com Crédito). **P2-4 Sparkline universal coverage**: Lazy gates `enabled: sectionVisible(...)` removidos dos 7 bundles (ntnb, breakeven, tesouro, credpriv + 5y variants) — KPICards agora populam trend desde mount. **P2-5 Empty states contextuais**: Yield Curve snapshot e NTN-B Term Structure renderizam `InlineEmpty` com mensagem contextual quando séries retornam zeros fallback. **P2-6 Breadcrumbs enriquecidos**: Main breadcrumb inclui label da section ativa (ex: "Renda Fixa > Títulos Públicos") com primeira crumb linkada para `/renda-fixa`. **P2-7 OnboardingTour step RF**: Descrição enriquecida — "30 indicadores BACEN SGS, curva DI 9 vértices, NTN-B 4 vencimentos, BEI 3 horizontes, calculadoras de bonds e carteira RF, simulador Tesouro Direto, Crédito Privado AA/A + CRA/CRI". Icon accent atualizado para #10B981. **P2-3 FeedbackWidget scoped by section**: Validado como já implementado via Section Nav Refactor (commit `b3b3350`) — zero code change. Build clean (tsc 0 errors, vite 5.49s). Chunk HubRendaFixa 101.14 → 102.19 kB / gzip 26.29 → 26.62 kB (+1.05 kB para 6 features). Deferidos pós-beta: P2-2 A11y axe-core audit (requer WCAG 2.1 AA sweep), P2-8 PDF export templates dedicados. Push pendente junto com `cdfff09` + `30729a8` + `bb637eb` + `27cc543` (credenciais GitHub indisponíveis — Lucas deve rodar `git push origin main` para publicar os 5 commits).
...
### P1-1 · COPOM event overlay ✅ (commit `27cc543`, 20/04/2026)
MacroChartEvent em 4 charts críticos: Selic vs CDI, DI 30d vs 360d, NTN-B 2029 vs 2035, IMA-B Proxy. Pattern idêntico ao P1-2 do Crédito e Macro #14. Implementado via `useMonetaryEvents("COPOM")` + `copomEvents` useMemo (map de `MonetaryEvent` → `MacroChartEvent` com label `COPOM ±bps → rate_after%` e kind hike/cut/hold). Zero infra nova — MacroChart v3 já suportava a prop `events?: MacroChartEvent[]` nativamente.
...
### P1-6 · Quick-chart buttons "abrir em /hub/macro" e "abrir em /hub/credito" ✅ (commit `27cc543`, 20/04/2026)
Cross-module navigation entregue em 3 superfícies complementares: (i) strip **Navegação cruzada** no topo da seção Visão Geral com 4 cards — Macro/Monetária, Macro/Inflação, Crédito/Preço, Crédito/Volume — preservando `?period=` vigente; (ii) pills inline na seção **Taxas & Curva** antes do Yield Curve snapshot linkando para `/hub/macro?section=monetaria` e `/hub/macro?section=expectativas`; (iii) pills inline na seção **Crédito Privado** antes dos charts de spread linkando para `/hub/credito?section=preco` e `/hub/credito?section=risco`. Todas as superfícies usam `Link` do react-router-dom (zero page reload) + `no-print` class (excluídas do PDF export). Accent emerald `#10B981` para preservar identidade RF.
