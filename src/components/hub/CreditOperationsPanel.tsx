...
function CustomTooltip({ active, payload, label }: { active?: boolean; payload?: TooltipPayload[]; label?: string }) {
  ...
  return (
    <div className="bg-[#111] border border-[#1a1a1a] rounded-lg p-2.5 shadow-xl">
    ...
    <button
      ...
      className={`px-2.5 py-1 text-[10px] font-mono rounded-md transition-all whitespace-nowrap ${
        active
          ...
          : "bg-[#111] text-zinc-500 border border-[#1a1a1a] hover:text-zinc-300 hover:border-[#252525]"
      }`}
    >
      {label}
...
function OperationCard({ mod, color, saldoData, taxaData, inadimData }: {
  ...
  return (
    <div className="bg-[#0f0f0f] border border-[#1a1a1a] rounded-lg p-3 hover:border-[#252525] transition-colors">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-1 rounded hover:bg-[#1a1a1a] text-zinc-600 hover:text-zinc-300 transition-colors"
            title="Exportar CSV"
          >
    ...
    <div className="bg-[#0f0f0f] border border-[#1a1a1a] rounded-lg overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.id} className={`border-b border-[#111] ${i % 2 === 0 ? "bg-[#0a0a0a]" : ""} hover:bg-[#111] transition-colors`}>
...
export function CreditOperationsPanel() {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 text-[9px] font-mono text-zinc-500 hover:text-zinc-300 border border-[#1a1a1a] rounded hover:border-[#252525] transition-colors"
            >
              <RotateCcw className="w-2.5 h-2.5" />
              Reset
            </button>
            <div className="flex items-center gap-0.5 bg-[#0a0a0a] border border-[#1a1a1a] rounded-md p-0.5">
              {PERIODS.map((p) => (
                <button
                  key={p}
                  onClick={() => setPeriod(p)}
      ...
      {/* ─── Footer ─── */}
      <div className="text-[8px] text-zinc-700 font-mono flex items-center justify-between pt-2 border-t border-[#141414]">
